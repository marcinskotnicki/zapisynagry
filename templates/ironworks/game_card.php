<?php
/* =============================================================================
 *  templates/ironworks/game_card.php — one game as a map plate.
 * -----------------------------------------------------------------------------
 *  THE MARKUP IS classic's, DELIBERATELY AND VERBATIM. The brief was "loosely
 *  based on the classic template", and classic's card is already the right
 *  SHAPE for this: a dark panel of stacked labelled bands beside a light player
 *  panel. Those bands are what become the ribbon name-plates of an industrial
 *  railway map (WOLVERHAMPTON, BIRMINGHAM), and the numbered slots become the
 *  little building tiles sitting on a city. All of that is css/style.css — not
 *  one line of markup differs.
 *
 *  Copied rather than shared because tpl_file() resolves per theme and falls
 *  back only to light — there is no way for one theme to inherit another's
 *  partial. tests/test_ironworks.php pins the two byte-identical below their
 *  headers, so the copy cannot rot silently, and pins the same control set
 *  every fork is held to: classic itself once lost its anti-bot field this way
 *  and broke commenting for months.
 *
                    ...
                    <?php if (!empty($g['comments'])): ?>
                        <ul class="comment-list">
                            <?php foreach ($g['comments'] as $c): ?>
                                <li><span class="c-name"><?= e($c['name']) ?>:</span> <?= nl2br(e($c['comment'])) ?><?= comment_edit_html($c, 'game') ?><?= comment_delete_html($c, 'game') ?></li>
                            <?php endforeach; ?>
                        </ul>
                    <?php endif; ?>
                    <?php if (!$readonly): ?>
        ...
        <div class="gc-players">
            ...
            <?php foreach ($reserves as $p): // reserves listed under the slots ?>
                <div class="gc-slot gc-reserve">
                    <span><strong><?= e($p['name']) ?></strong></span>
