<?php
/* =============================================================================
 *  templates/light/add_game_own_list.php — pick a game from your OWN library.
 * -----------------------------------------------------------------------------
 *  Stands exactly where the BGG search results stand: choose one, and the next
 *  screen is the ordinary game form, prefilled. Everything after this point is
 *  unchanged, which is the whole design — this replaces the SEARCH step, not
 *  the form.
 *
 *  A near-copy of add_game_club_list.php on purpose: the two lists differ only
 *  in whose games they hold and which query parameter carries the choice.
 *
 ...
 *    $games — this member's own active games, alphabetically.
 * ============================================================================= */
?>
<div class="card">
    ...
    <div class="field club-filter" hidden>
        ...
        <ul class="lib-list club-pick-list">
            <?php foreach ($games as $g): ?>
                <li class="lib-item">
                    <a class="club-pick" href="add_game.php?table=<?= (int)$table['id'] ?>&amp;mine=<?= (int)$g['id'] ?>">
    ...
    <div class="form-actions">
        <a class="btn" href="add_game.php?table=<?= (int)$table['id'] ?>"><?= e(t('back')) ?></a>
    </div>
</div>
