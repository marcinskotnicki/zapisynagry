                    ...
                    <ul class="player-list">
                        <?php foreach ($g['players'] as $p): ?>
                            <li class="player<?= (int)$p['is_reserve'] === 1 ? ' player-reserve' : '' ?>">
                                <?= e($p['name']) ?><?php if ($p['knows_rules'] !== null && $p['knows_rules'] !== ''): // rules-knowledge dot: green knows / amber somewhat / red doesn't; tooltip spells it out ?><?= rules_icon_html(rules_tone($p['knows_rules']), knows_rules_label($p['knows_rules'])) ?><?php endif; ?><?php if (is_admin() && !empty($p['user_id'])): // admin-only: entry is BOUND to an account (created while its owner was logged in) ?><span class="p-acct" title="<?= e(t('player_account_bound', $p['account_name'] ?? ('#' . (int)$p['user_id']))) ?>">@</span><?php endif; ?><?php if ((int)$p['is_reserve'] === 1): ?> <span class="reserve-tag"><?= e(t('reserve_tag')) ?></span><?php endif; ?>
                                ...
                                <?php /* "Signed up by X" — see player_signed_up_by() for which
                                       ...
                                       * understands why they get no remove button. */
